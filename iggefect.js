(function($){
	$.fn.iggefect = function(effect, value){
		...
		this.convolute32 = function(pixels, weights, opaque){
			var side = Math.round(Math.sqrt(weights.length));
			var halfSide = Math.floor(side/2);
			...
			for (var y=0; y<h; y++) {
				for (var x=0; x<w; x++) {
					...
					dst[dstOff] = (r > 255 ? 255 : r);
					dst[dstOff+1] = (g > 255 ? 255 : g);
					dst[dstOff+2] = (b > 255 ? 255 : b);
					dst[dstOff+3] = (a + alphaFac*(255-a) > 255 ? 255 : a + alphaFac*(255-a));
				}
			}
			return output;
		}
